...
- `lsof`, `git`, and `ps` are present and produce output the port scanner can parse, so the core port-scanning feature works. For the panel to display anything, a dev server must be listening on a port in the range **1024–9999** (ports outside that range are filtered out). Quick smoke test: run `npm run demo:server` in one terminal, then `DISPLAY=:1 npm run dev` in another and click the tray icon — `Demo App` should appear within ~3s. Use `LB_DEMO=1 npm run dev` to keep the panel open for screenshots.
